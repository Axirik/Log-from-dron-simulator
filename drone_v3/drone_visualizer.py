...
state = {
    ...
    'cur_state': 'LANDED', 'cur_yaw': 0.0,
    'packets': 0, 'running': True,
    'mode': 'LIVE', 'flight_start': None,
    'replay_start': '--', 'replay_end': '--', 'replay_total': 0.0,
}
        ...
        try:
            ...
            with lock:
                ...
                state.update({
                    ...
                    'cur_roll':  math.degrees(roll),
                    'cur_pitch': math.degrees(pitch),
...
def load_csv(filepath):
    ...
    for row in rows:
        s = row.get('state', row.get('state ', 'FLYING')).strip()
        with lock:
    ...
    with lock:
        if state['xs']:
            ...
            state.update({
                ...
                'cur_y':     state['ys'][-1],
                'cur_z':     state['zs'][-1],
                'cur_roll':  math.degrees(state['rolls'][-1]),
                'cur_pitch': math.degrees(state['pitches'][-1]),
                'cur_state': 'REPLAY',
            })

    print(f"[REPLAY] Загружено {state['packets']} точек")
    print(f"[REPLAY] Уникальные состояния: {set(state['states'])}")
...
class App:
    ...
    def _build_ui(self):
        ...
        for tab_name, tab_key in [('ИНФО','info'),('ЛЕГЕНДА','legend'),('СТАТ','stats')]:
            btn = tk.Button(tabs_frame, text=tab_name, bg='#21262d', fg=FG_DIM,
        ...
        tk.Radiobutton(src_frame, text='LIVE', variable=self.mode_var,
                       ...
                       font=('Courier', 9), command=self._set_live).pack(side=tk.LEFT)
        tk.Radiobutton(src_frame, text='REPLAY', variable=self.mode_var,
                       value='REPLAY', bg=BG2, fg=ORANGE, selectcolor=BG,
                       font=('Courier', 9), command=self._set_replay).pack(side=tk.LEFT)
    ...
    def _set_replay(self):
        ...
        if filepath:
            state['mode'] = 'REPLAY'
            self._clear()
            threading.Thread(target=load_csv, args=(filepath,), daemon=True).start()
        else:
    ...
    def _switch_flight_mode(self):
        ...
        self.ctrl_sock.sendto(f"MODE,{mode}".encode(), ('127.0.0.1', 14553))
        print(f"[FLIGHT MODE] {mode}")

    # ══════════════════════════════════════
    #  РИСОВАНИЕ ТРАЕКТОРИИ — одна функция для LIVE и REPLAY
    # ══════════════════════════════════════
    def _draw_trajectory(self, ax, xs, ys, zs, states):
        n = len(xs)
        if n < 2:
            return

        ax.set_zlim(0, max(zs) + 1 if max(zs) > 0 else 5)

        i = 0
        while i < n - 1:
            cs    = states[i].strip() if i < len(states) else 'FLYING'
            color = STATE_COLORS.get(cs, BLUE)
            j     = i + 1
            while j < n and states[j].strip() == cs:
                j += 1
            ax.plot(xs[i:j+1], ys[i:j+1], zs[i:j+1],
                    color=color, linewidth=1.5)
            i = j

        ax.scatter(xs[-1], ys[-1], zs[-1], color=RED,   s=60, zorder=5)
    ...
    def _update(self):
        try:
            ...
            self._draw_trajectory(self.ax3d, xs, ys, zs, states)
            ...
            for spine in self.ax_alt.spines.values():
                spine.set_edgecolor(BORDER)
            if n > 1:
                tail = zs if mode == 'REPLAY' else zs[-TAIL:]
            ...
            for spine in self.ax_rp.spines.values():
                spine.set_edgecolor(BORDER)
            if len(rolls) > 1:
                r = rolls   if mode == 'REPLAY' else rolls[-TAIL:]
                p = pitches if mode == 'REPLAY' else pitches[-TAIL:]
    ...
    def _draw_drone_indicator_ax(self, roll, pitch, yaw):
        ...
        arm = 0.55
        bx  = math.sin(math.radians(roll))  * 0.2
        by  = math.sin(math.radians(pitch)) * 0.2

        for base_angle in [135, 45, 225, 315]:
            angle = math.radians(base_angle) + yaw
            ex = bx + arm * math.cos(angle)
            ey = by + arm * math.sin(angle)

            ax.plot([bx, ex], [by, ey],
                    color='#30363d', lw=2.5, solid_capstyle='round')

            scale_x = 1.0 - abs(math.sin(math.radians(roll)))  * 0.4
            scale_y = 1.0 - abs(math.sin(math.radians(pitch))) * 0.4
            ax.add_patch(matplotlib.patches.Ellipse(
                (ex, ey), width=0.22*scale_x, height=0.22*scale_y,
                color='#21262d', ec=BLUE, lw=1.0))
            ax.add_patch(matplotlib.patches.Circle((ex, ey), 0.035, color=BLUE))

        ax.add_patch(matplotlib.patches.Circle(
            (bx, by), 0.10, color='#161b22', ec='#30363d', lw=1.2))
        ...
        ax.text(0, -0.92,
                f'R{roll:+.0f}° P{pitch:+.0f}° Y{math.degrees(yaw):+.0f}°',
